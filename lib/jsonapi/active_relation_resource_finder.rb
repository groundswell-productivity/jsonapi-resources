...
module JSONAPI
  module ActiveRelationResourceFinder
    ...
    module ClassMethods
      ...
      # The ActiveRecordResourceFinder uses an `ActiveRecord_Relation` class from the model as the starting point for
      # retrieving models. From this `ActiveRecord_Relation` filters, sorts and joins are applied as needed.
      ...
      # Returns an `ActiveRecord_Relation` from the model. This is the default starting for the other `records` methods
      # below and is not directly used for accessing model data by this class. Overrides give a single place to affect
      # the `ActiveRecord_Relation` used for the resource
      def records_base(_options = {})
        _model_class.all
      end

      # Returns an `ActiveRecord_Relation` from the model. Returns the ActiveRecord_Relation used for finding user
      # requested models. This may be overridden to enforce permissions checks on the request.
      def records(options = {})
        records_base(options)
      end

      # Returns an `ActiveRecord_Relation` from the model. Returns the ActiveRecord_Relation used for populating the
      # ResourceSet. Only resources that have been previously identified through the `records` method will be accessed.
      # Thus it should not be necessary to reapply permissions checks. However if the model needs to include other
      # models adding `includes` is appropriate
      def records_for_populate(options = {})
        records_base(options)
      end

      # Returns an `ActiveRecord_Relation` from the model. Returns the ActiveRecord_Relation used for the finding
      # related resources. Only resources that have been previously identified through the `records` method will be
      # accessed and used as the basis to find related resources. Thus it should not be necessary to reapply
      # permissions checks.
